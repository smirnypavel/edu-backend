...
export class ProfileService {
  ...
  async getDashboard(userId: string) {
    try {
      ...
      return {
        ...
        activeCourses: user.enrolledCourses.length,
  ...
  async getLearningHistory(userId: string) {
    try {
      ...
      this.logger.error(`Ошибка при получении истории обучения: ${error.message}`);
      throw new HttpException(
  ...
  private calculateTotalProgress(enrolledCourses: any[]): number {
    try {
      ...
      return enrolledCourses.reduce((total, course) => total + course.progress, 0) / enrolledCourses.length;
    } catch (error) {
  ...
  private calculateTotalScore(enrolledCourses: any[]): number {
    try {
      return enrolledCourses.reduce((total, course) => total + (course.score || 0), 0);
    } catch (error) {
  ...
  private getLastActivity(course: any) {
    try {
      ...
      this.logger.error(`Ошибка при получении последней активности: ${error.message}`);
      return null;
    }
  }
