...
export class CoursesService {
  ...
  async updateCourse(id: string, updateData: Partial<CreateCourseDto>): Promise<Course> {
    try {
      const course = await this.courseModel
  ...
  async addLesson(courseId: string, lessonDto: CreateLessonDto): Promise<Course> {
    try {
  ...
  async updateLesson(courseId: string, lessonId: string, lessonDto: Partial<CreateLessonDto>): Promise<Course> {
    try {
      const course = await this.courseModel.findById(courseId);
      if (!course) {
        throw new NotFoundException('Курс не найден');
  ...
  async evaluateCode(courseId: string, lessonId: string, submission: { code: string; language: string }) {
    try {
      const course = await this.courseModel.findById(courseId);
      if (!course) {
        throw new NotFoundException('Курс не найден');
  ...
  async evaluateTest(courseId: string, lessonId: string, submission: TestSubmissionDto) {
    try {
      const course = await this.courseModel.findById(courseId);
      if (!course) {
        throw new NotFoundException('Курс не найден');
      ...
      const timeBonus = this.calculateTimeBonus(submission.timeTaken, timeLimit);
  ...
  private checkTestAnswers(
    ...
    return submissions.map(submission => {
      ...
      if (!test) {
        throw new NotFoundException(`Вопрос ${submission.questionId} не найден`);
      }
  ...
  private calculateTestScore(results: any[], timeBonus: number): number {
    try {
      const baseScore = results.reduce((total, result) => total + result.points, 0);
  ...
  private async saveTestResults(courseId: string, lessonId: string, results: any): Promise<void> {
    try {
      ...
      this.logger.error(`Ошибка сохранения результатов теста: ${error.message}`);
      throw new InternalServerErrorException('Ошибка при сохранении результатов теста');
    }
  }
}